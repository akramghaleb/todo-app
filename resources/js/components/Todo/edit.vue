    ...
    <div id="create-todo">
        ...
        <div class="w-full p-4 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md sm:p-6 lg:p-8 dark:bg-gray-800 dark:border-gray-700">
            <form class="space-y-6" @submit.prevent="edit()">
                <h5 class="text-xl font-medium text-gray-900 dark:text-white">Edit todo</h5>
                <div>
...
name: "create",
    ...
    methods:{
        async edit(){
            ...
            this.errors = {};
            await axios.put('/api/todo/update/'+this.$route.params.id, this.form)
                .then(res => {
    ...
    async beforeMount() {
        ...
        this.errors = {};
        await axios.get('/api/todo/'+this.$route.params.id)
            .then(res => {
                this.form.name = res.data.name;
                this.form.description = res.data.description;
                this.form.completed = res.data.completed.toString();
            })
            .catch(error => {
                this.errors = error.response.data.errors
            });
